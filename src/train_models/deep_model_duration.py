        ...
        for file in files:
            if file == 'calc.json':
                with open(os.path.join(root, file), 'r') as f:
                    ...
                    if duration < 200:
                        # выбираем, какие параметры использовать для обучения
                        data.append(
                            {param: json_data[param] for param in train_params + predict_parameters}
                        )
...
import pickle
with open(os.path.join(pth_dir, 'scaler_deep_model_duration.pkl'), 'wb') as f:
    pickle.dump(scaler, f)

# Использую X_train_scaled, y_train напрямую
train_dataset = DurationDataset(X_train_scaled, y_train)  # возвращаем к duration, а не классам
...
for epoch in range(num_epochs):
    ...
    # Считаем MAE и MRE для обеих выборок (для последующего сохранения и анализа)
    ...
    if val_loss < best_val_loss:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(pth_dir, 'best_regression_deep_model_10s.pth'))
    else:
        patience_counter += 1
...
import pickle
with open(os.path.join(pth_dir, 'duration_train_losses.pkl'), 'wb') as f:
    pickle.dump(train_losses, f)
with open(os.path.join(pth_dir, 'duration_val_losses.pkl'), 'wb') as f:
    pickle.dump(val_losses, f)
with open(os.path.join(pth_dir, 'duration_train_maes.pkl'), 'wb') as f:
    pickle.dump(train_maes, f)
with open(os.path.join(pth_dir, 'duration_val_maes.pkl'), 'wb') as f:
    pickle.dump(val_maes, f)
with open(os.path.join(pth_dir, 'duration_train_mres.pkl'), 'wb') as f:
    pickle.dump(train_mres, f)
with open(os.path.join(pth_dir, 'duration_val_mres.pkl'), 'wb') as f:
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(img_dir, 'duration_mae_history.png'))
plt.savefig(os.path.join(img_dir, 'duration_mae_history.svg'))
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(img_dir, 'duration_mre_history.png'))
plt.savefig(os.path.join(img_dir, 'duration_mre_history.svg'))
plt.close()
...
print(f'Среднее значение duration в датасете: {df["duration"].mean():.2f}') 
...
plt.tight_layout()
plt.savefig(os.path.join(img_dir, 'scatter_true_vs_pred_deep_model_duration.png')) 
plt.savefig(os.path.join(img_dir, 'scatter_true_vs_pred_deep_model_duration.svg'))
